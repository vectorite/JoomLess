...
jQuery(document).ready(function() {
	...
	/* Animation for Make a Gift */
	jQuery('#giftFade').delay(6000).fadeOut(1000);
	jQuery('.wordMake').delay(1000).fadeIn(1000);
	jQuery('.wordA').delay(2000).fadeIn(1000);
	jQuery('.wordGift').delay(3000).fadeIn(1000);
	jQuery('.wordSymbol').delay(4000).fadeIn(1000);
	...
	jQuery('#homeSplash').css("display", "block"); //display the welcome text

	jQuery('#homeSplash img').click(function() { //when clicked X do something
		var jQuerythis = jQuery(this);
		if (jQuerythis.hasClass('clicked')) { //check that if it was cliked before or not

		}	else {  //else means that link wasn't clicked
     	jQuery(this).addClass('clicked'); //add class that the link is clicked and shot the footer with delay
     	jQuery('#homeSplash').delay(200).fadeOut(1000)

			jQuery.cookie('welcome2', 'clicked'); //add cookie setting that user has clicked
		}
		return false;
  });

	// COOKIES
	var cookie_mesage_clicked = jQuery.cookie('welcome2'); //set the variable for cookie

	if (cookie_mesage_clicked == 'clicked') { //if the cookie_mesage equals clicked, means user opted to hide message
		jQuery('#homeSplash').css("display", "none");
	};

	jQuery.cookie('welcome', 'hide'); //add cookie to hide the first message after load (on refresh it will be hidden)
